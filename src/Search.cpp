...
Search::Search(int id, int ** m, int posRX, int posRY, int posIX, int posIY, int tamX, int tamY, int ratioX, int ratioY, tiposEstrategias es, int col, int posFX, int posFY){
	_mapa = m;
	_id = id;
	_posActX = posIX;
	_posActY = posIY;
...
vector<State*> Search::getSolucion() const{
	return _solucion;
}

int Search::costePosicion(const int posX, const int posY) {
	return 3;
}
...
double Search::estrategia(const int coste, const int posX, const int posY) const{
	return -(coste + heuristica(posX,posY));
}

double Search::heuristica(const int posX, const int posY) const{
	switch(_estrategia){
...
int Search::costeMinimo() const{
	return 1;
}

bool Search::puedoGenerar(movimientos mov, int * c, int * f, int * coste){
    int inicio [6];
    int fin [6];
    if (_posRX<0.5){
        if (_posRY<0.5){
...
void Search::visitar(int posX, int posY, int coste){
	_visitados[posX][posY] = coste + heuristica(posX, posY);
}

bool Search::visitado(int posX, int posY, int coste) const{
...
void Search::ejecutar(){
	int coste, posAuxX, posAuxY;
	bool expandir = true;
	int mov;
	Nodo candidato(new State(_posActX, _posActY, NORTE), 0, 0, 0);
	visitar(_posActX, _posActY, candidato.getCoste());
	_frontera.push(candidato);
	cout<<_id<<":::::"<<_posFinX<<"::::"<<_posFinY<<endl;
	while(expandir){
		candidato = _frontera.top();
		_frontera.pop();
		_posActX = candidato.getState()->getX();
		_posActY = candidato.getState()->getY();
		expandir = !fin();
		if(expandir){
			for(int i = 0; i < 8; i++){ //8 tipos de movimientos
				...
				if(puedoGenerar(static_cast<movimientos>(mov), &posAuxX, &posAuxY, &coste)){ //Tambien se comprueba si esta visitada
					...
					Nodo hijo(new State(posAuxX, posAuxY, static_cast<movimientos>(mov)), coste, &candidato, estrategia(coste, posAuxX, posAuxY));
					_frontera.push(hijo);
...
bool Search::haySolucion() const{
	return _sol;
}

void Search::generarSolucion(Nodo * sol){
	_costeSol = sol->getCoste();
	while(sol->getPadre()){
        _solucion.push_back(sol->getState());
        sol = sol->getPadre();
	}
	//_solucion.push_back(sol->getState());
}

int Search::getNelem() const{
	return _nElem;
}

int Search::getCosteSol() const{
	return _costeSol;
}
